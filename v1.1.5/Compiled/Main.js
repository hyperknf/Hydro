...
function Run(path) {
    return __awaiter(this, void 0, void 0, function () {
        var https, fs, parser, environment, run_path, name_1, entry, entry_absolute, description, author, version, absolute_path, data, main, config, input, program, result;
        var _this = this;
                ...
                case 1:
                    ...
                    if (path) {
                        if (path == ".init") {
                            ...
                            name_1 = process.argv[3] || prompt("Enter module name: ");
                            entry = process.argv[4] || prompt("Enter entry file: ");
                            if (entry.includes("\\") || entry.includes("/"))
                                throw new Error("The entry name you've entered is a directory");
                            entry_absolute = Path.resolve("./".concat(entry));
                            if (FileSystem.existsSync(entry_absolute))
                                throw new Error("File ".concat(entry_absolute, " already exists"));
                            description = process.argv[5] || prompt("Enter description: ");
                            author = process.argv[6] || prompt("Enter author: ");
                            version = process.argv[7] || prompt("Enter module version: ");
                            if (FileSystem.existsSync(Path.resolve("./hydro_config.json")))
                                throw new Error("hydro_config.json already exists in the current directory");
                            ...
                            FileSystem.writeFileSync(Path.resolve("./".concat(entry)), "");
