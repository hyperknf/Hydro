    ...
    while (source_characters.length > 0) {
        if (source_characters[0] == "(") {
            ...
            if (is_integer(source_characters[0])) {
                let number: string = ""
                while (source_characters.length > 0 && (is_integer(source_characters[0]) || source_characters[0] == ".")) number += source_characters.shift()
                if (number.charAt(number.length - 1) == "." || number.split(".").length >= 3) throw new Error("Unexpected \".\" in number")
                tokens.push(token(number, TokenType.Number))
            } else if (is_alpha(source_characters[0]) || source_characters[0] == "_") {
                let identifier: string = ""
                while (source_characters.length > 0 && (is_alpha(source_characters[0]) || source_characters[0] == "_" || is_integer(source_characters[0]))) identifier += source_characters.shift()
